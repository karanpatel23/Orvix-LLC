  ...
  return <header className='fixed inset-x-0 top-0 z-50 px-4 pt-4'>
    <nav className={`containerX rounded-full transition-all ${scrolled ? 'glass' : 'bg-transparent'}`}>
      <div className='flex items-center justify-between px-5 py-3'>
        <Link href='/' aria-label='ORVIX home'><Logo /></Link>
        ...
        <div className='flex items-center gap-3'><Link href='/contact' className='hidden sm:inline-flex rounded-full bg-[#c6a56b] px-4 py-2 text-sm font-medium text-black'>Request Quote</Link><button className='lg:hidden' onClick={() => setOpen(v => !v)} aria-label='Toggle menu'>☰</button></div>
      </div>
    </nav>
